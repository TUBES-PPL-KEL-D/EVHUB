    ...
    <div class="grid grid-cols-1 gap-10"> 
        <div class="bg-slate-800/40 border border-slate-700/50 rounded-[2.5rem] p-8 backdrop-blur-sm shadow-xl"> 
            ...
            <div class="space-y-4"> 
                ...
                <div class="bg-slate-800/50 border border-slate-700/50 p-5 rounded-3xl flex flex-col justify-between gap-4"> 
                    ...
                    <div id="log-{{ $vendor->id }}" class="hidden border-t border-slate-700/50 pt-4 mt-1 bg-slate-900/30 p-4 rounded-2xl space-y-2 animate-fade-in">
                        <p class="text-xs font-black uppercase tracking-widest text-amber-400/80 mb-2">Pesan Pelanggaran Terbuku:</p>
                        @foreach($vendor->warnings as $index => $warning)
                        <div class="text-xs text-slate-300 flex justify-between items-start gap-4 border-b border-slate-800/60 pb-2 last:border-none">
...
<script>
    // Logika buka tutup riwayat pesan peringatan
    function toggleWarningLog(logId) {
        const logBlock = document.getElementById(logId);
        if(logBlock.classList.contains('hidden')) {

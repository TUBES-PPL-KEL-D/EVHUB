...
<div class="space-y-6 animate-fade-in-up pb-10"> 
    ...
    <div class="flex border-b border-slate-700/60 gap-2 mb-6 overflow-x-auto"> 
        <button id="tabBtn-overview" onclick="switchMainTab('overview')" class="main-tab-btn px-6 py-3.5 font-black text-xs tracking-widest uppercase border-b-2 transition-all duration-200 text-emerald-400 border-emerald-500 shrink-0"> Overview & Analytics </button> 
        <button id="tabBtn-verifikasi" onclick="switchMainTab('verifikasi')" class="main-tab-btn px-6 py-3.5 font-black text-xs tracking-widest uppercase border-b-2 transition-all duration-200 text-slate-500 border-transparent hover:text-slate-300 shrink-0"> Antrean Verifikasi ({{ count($pendingVendors ?? []) }}) </button> 
        <button id="tabBtn-manajemen" onclick="switchMainTab('manajemen')" class="main-tab-btn px-6 py-3.5 font-black text-xs tracking-widest uppercase border-b-2 transition-all duration-200 text-slate-500 border-transparent hover:text-slate-300 shrink-0"> Manajemen Vendor & Stasiun </button> 
    </div> 
    ...
    <div id="panel-verifikasi" class="main-panel-content space-y-6 hidden"> 
        <div class="bg-slate-900/40 border border-slate-700/50 rounded-[2rem] p-6 md:p-8 backdrop-blur-xl shadow-xl"> 
            <h2 class="text-xl font-bold text-white mb-6 flex items-center gap-2"> 
                <span class="w-2.5 h-2.5 rounded-full bg-amber-500"></span> Validasi Berkas Kemitraan Baru 
            </h2> 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4"> 
                ...
                <div class="flex items-center justify-between p-5 bg-slate-800/40 rounded-2xl border border-slate-700/30"> 
                    <div> 
                        ...
                        <p class="text-slate-400 text-xs mt-1">NPWP: <span class="font-mono tracking-wider text-amber-400">{{ $vendor->profile?->npwp ?? 'Belum Diinput' }}</span></p> 
                    </div> 
                    
                    <button type="button" 
                        class="bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-colors border border-blue-500/20" 
                        data-id="{{ $vendor->id }}" 
                        data-name="{{ $vendor->company_name }}" 
                        data-npwp="{{ $vendor->profile?->npwp ?? 'Belum Diinput' }}" 
                        data-email="{{ $vendor->user->email ?? 'Tidak ada email' }}" 
                        data-address="{{ $vendor->profile?->company_address ?? 'Alamat belum diisi' }}" 
                        data-pdf="{{ $vendor->legality_document_path ? asset('storage/' . $vendor->legality_document_path) : '' }}" 
                        data-status="Pending" 
                        onclick="openReviewModal(this)"> TINJAU 
                    </button> 
                </div> 
                @endforeach 
            </div> 
    ...
    <div id="panel-manajemen" class="main-panel-content space-y-8 hidden"> 
        <div class="flex justify-between items-center bg-slate-900/30 p-4 border border-slate-700/30 rounded-2xl"> 
            ...
            <div class="flex flex-wrap gap-3"> 
                <a href="{{ route('admin.withdrawals.index') }}" class="inline-flex items-center bg-blue-500/10 text-blue-400 border border-blue-500/20 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all"> Withdrawal Vendor </a> 
                <a href="{{ route('admin.export.spklu') }}" class="inline-flex items-center bg-emerald-500 text-slate-900 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-emerald-400 transition-all shadow-lg"> 
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg> Download Berkas Excel 
                </a> 
            </div> 
        </div> 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start"> 
            <div class="bg-slate-900/40 border border-slate-700/50 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl lg:col-span-1"> 
                <h2 class="text-lg font-bold text-white flex items-center gap-2 mb-6"> 
                    <span class="w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse"></span> Laporan Kendala Aktif 
                </h2> 
                        ...
                        <div class="bg-slate-800/40 border border-slate-700/40 p-5 rounded-2xl flex flex-col justify-between gap-4"> 
                            <div class="flex flex-col xl:flex-row justify-between items-center gap-4 w-full"> 
                                <div class="flex items-center space-x-4 w-full xl:w-auto"> 
                                    <div class="w-11 h-11 bg-emerald-500/10 rounded-xl flex items-center justify-center font-black text-emerald-400 text-base flex-shrink-0"> {{ substr($vendor->company_name, 0, 1) }} </div> 
                                    <div> 
                                ...
                                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full xl:w-auto items-center"> 
                                    ...
                                    <button type="button" 
                                        class="bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-colors border border-blue-500/20 w-full sm:w-auto text-center" 
                                        data-id="{{ $vendor->id }}" 
                                        data-name="{{ $vendor->company_name }}" 
                                        data-npwp="{{ $vendor->profile?->npwp ?? 'Belum Diinput' }}" 
                                        data-email="{{ $vendor->user->email ?? 'Tidak ada email' }}" 
                                        ...
                                        onclick="openReviewModal(this)"> BERKAS 
                                    </button> 
    ...
    <div id="reviewModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/90 backdrop-blur-md opacity-0 transition-opacity duration-300"> 
        <div id="reviewModalContent" class="bg-slate-900 border border-slate-700/80 w-11/12 max-w-6xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col transform scale-95 transition-transform duration-300 max-h-[90vh]"> 
                    ...
                    <div> 
                        ...
                        <div class="space-y-4"> 
                            ...
                            <div class="bg-slate-800/50 p-4 rounded-2xl border border-slate-700/50"> 
                                ...
                                <p id="modalNpwp" class="text-amber-400 font-mono text-sm tracking-widest">-</p> 
                            </div> 
                            <div class="bg-slate-800/50 p-4 rounded-2xl border border-slate-700/50"> 
            ...
            <div class="p-6 border-t border-slate-800 bg-slate-900/90 flex justify-end gap-4"> 
                <button id="btnModalCloseNeutral" type="button" onclick="closeReviewModal()" class="hidden bg-slate-800 text-slate-300 border border-slate-700 hover:text-white px-8 py-3 rounded-2xl font-black tracking-widest uppercase text-sm"> Tutup Tinjauan </button> 

	...
	<div class="mx-auto max-w-4xl">
		...
		<div class="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
			<div class="border-b border-slate-200 bg-slate-50 px-6 py-4">
				<h2 class="text-lg font-semibold text-slate-900">Form Profil Vendor</h2>
			</div>
			...
			<form action="{{ route('vendor.profile.store') }}" method="POST" class="space-y-6 px-6 py-6">
				...
				<div class="flex flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
					...
					<div class="flex gap-3">
						...
						<button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700">Simpan Profil</button>
					</div>
				</div>
			</form>
		</div>

    ...
    <div class="flex flex-col lg:flex-row gap-6 h-[calc(100vh-200px)] min-h-[500px]"> 
        <div class="relative w-full lg:w-2/3 h-full bg-slate-100 rounded-[2rem] overflow-hidden shadow-sm border border-slate-700/50"> 
            <div class="absolute top-6 left-6 z-[1000] flex flex-col sm:flex-row gap-3 w-full max-w-md pr-6"> 
                <input type="text" id="search-spklu" placeholder="Cari lokasi, nama stasiun..." class="w-full bg-slate-900/95 backdrop-blur border border-slate-700 text-white placeholder-slate-400 text-sm rounded-full focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 block py-3 px-6 shadow-lg outline-none transition-all"> 
                <select id="filter-status" class="bg-slate-900/95 backdrop-blur border border-slate-700 text-white text-sm rounded-full focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 block py-3 px-6 shadow-lg outline-none cursor-pointer transition-all"> 
        ...
        <div class="w-full lg:w-1/3 h-full bg-slate-900 rounded-[2rem] shadow-sm border border-slate-700 flex flex-col overflow-hidden"> 
            <div class="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900 shrink-0"> 
                ...
                <span id="list-count" class="text-xs font-bold px-3 py-1 bg-emerald-900/50 text-emerald-400 rounded-full border border-emerald-800"> Memuat... </span> 
            </div> 
            <div id="station-list" class="flex-1 overflow-y-auto p-4 flex flex-col gap-4 custom-scrollbar"></div> 
        </div> 
    </div> 
</div> 
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderUserLocation(lat, lng, panTo = false, showPopup = false) {
        if (userMarker) {
            ...
            userMarker.bindPopup('<div class="text-center font-bold text-blue-600 px-3 py-1">Lokasi Anda Saat Ini</div>', {closeButton: false, className: 'modern-user-popup', offset: [0, -10] }); 
        } 
        if (panTo) { map.flyTo([lat, lng], 15, { animate: true, duration: 1.5 }); } 
        if (showPopup) {
            userMarker.openPopup(); 
            setTimeout(() => { if (userMarker && userMarker.isPopupOpen()) { userMarker.closePopup(); } }, 5000); 
        } 
    } 
    ...
    function initUserLocation() {
        if (navigator.geolocation) {
            ...
            }, (error) => { console.log("Lokasi gagal ditarik otomatis."); }, { enableHighAccuracy: true }); 
            
            navigator.geolocation.watchPosition((position) => {
                currentUserLat = position.coords.latitude; 
                currentUserLng = position.coords.longitude; 
                renderUserLocation(currentUserLat, currentUserLng, false, false); 
            }, (error) => { console.log("Gagal melacak lokasi."); }, { enableHighAccuracy: true }); 
        } 
    } 
    initUserLocation(); 
    ...
    window.getRoute = function(destLat, destLng, destName) {
        ...
        routingControl = L.Routing.control({
            waypoints: [L.latLng(currentUserLat, currentUserLng), L.latLng(destLat, destLng) ], 
            router: L.Routing.osrmv1({ language: 'id', profile: 'driving' }), 
            lineOptions: {styles: [{color: '#3b82f6', opacity: 0.5, weight: 6}]}, 
        ...
        routingControl.on('routesfound', function(e) {
            ...
            var timeText = totalMinutes > 60 ? Math.floor(totalMinutes / 60) + " Jam " + (totalMinutes % 60) + " Mnt" : totalMinutes + " Menit"; 
        ...
        fetch(url).then(response => response.json()).then(data => {
            ...
            data.forEach(spklu => {
                if (spklu.latitude && spklu.longitude) {
                    ...
                    let portsText = spklu.charger_machines && spklu.charger_machines.length > 0 ? spklu.charger_machines.map(m => m.connector_type).join(', ') : 'Port tidak tersedia'; 
                    let distanceHTML = ''; 
                    if (currentUserLat && currentUserLng) {
                    ...
                    // PERBAIKAN POIN 2: MEMBUAT SUB-TEMPLATE RATING ULASAN UNTUK DAFTAR SIDEBAR KANAN
                    let listRatingHTML = '';
                    if (spklu.review_count > 0) {
                        listRatingHTML = `
                            <div class="flex items-center gap-1 mt-1 mb-2 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-lg w-fit">
                                <svg class="w-3 h-3 text-amber-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                <span class="text-xs font-black text-amber-400">${parseFloat(spklu.avg_rating).toFixed(1)}</span>
                                <span class="text-[10px] text-slate-400">(${spklu.review_count} Ulasan)</span>
                            </div>
                        `;
                    } else {
                        listRatingHTML = `
                            <div class="flex items-center gap-1 mt-1 mb-2 bg-slate-900/50 border border-slate-700/50 px-2 py-0.5 rounded-lg w-fit">
                                <svg class="w-3 h-3 text-slate-600" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                <span class="text-[10px] text-slate-500">Belum ada ulasan</span>
                            </div>
                        `;
                    }
                        ...
                        <div class="bg-slate-800 rounded-2xl p-5 border border-slate-700 hover:border-emerald-500 transition-all shadow-sm group"> 
                            <div class="flex justify-between items-start mb-2"> 
                                <div class="flex-1 pr-3"> 
                                    ...
                                    <p class="text-slate-400 text-[11px] line-clamp-1 mb-1">${spklu.address || '-'}</p> 
                                    ${listRatingHTML}
                                </div> 
                            ...
                            <div class="flex gap-2 mt-2 pt-4 border-t border-slate-700/50"> 
                                ...
                                <a href="/rider/spklu/${spklu.id}" class="p-2.5 bg-slate-700 hover:bg-slate-600 rounded-xl text-slate-300 hover:text-white transition-colors border border-slate-600" title="Lihat Fasilitas & Ulasan"> 
                                ...
                                <button onclick="getRoute(${spklu.latitude}, ${spklu.longitude}, '${spklu.name}')" class="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-2.5 rounded-xl transition-colors flex items-center justify-center gap-2"> Gas Ke Sini </button> 
                            </div> 
                        </div>
                    `; 
                    ...
                    let popupContent = `
                        <div class="p-2 min-w-[200px] font-sans"> 
                            ...
                            <div class="flex justify-between items-center mt-3 pt-3 border-t border-slate-200"> 
                                ...
                                <button onclick="getRoute(${spklu.latitude}, ${spklu.longitude}, '${spklu.name}')" class="text-[10px] font-bold bg-emerald-100 text-emerald-700 px-3 py-1.5 rounded-lg hover:bg-emerald-200 transition-colors"> Gas Ke Sini </button> 
                            </div> 
                            <a href="/rider/spklu/${spklu.id}" class="block w-full bg-slate-800 hover:bg-slate-700 text-white text-center font-bold text-[10px] py-1.5 px-3 rounded-lg transition-colors mt-2 uppercase tracking-widest"> Fasilitas & Ulasan </a> 
                        </div>
                    `; 
...
.modern-user-popup .leaflet-popup-content-wrapper { border-radius: 9999px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); border: 1px solid #bfdbfe; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(4px); } 
.modern-user-popup .leaflet-popup-tip { background: rgba(255, 255, 255, 0.95); } 

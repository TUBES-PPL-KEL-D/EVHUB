        ...
        function renderUserLocation(lat, lng, panTo = false, showPopup = false) {
            if (userMarker) {
                ...
                userMarker.bindPopup('<div class="text-center font-bold text-blue-600 px-3 py-1">Lokasi Anda Saat Ini</div>', {
                    closeButton: false, className: 'modern-user-popup', offset: [0, -10]
                });
            }
        ...
        function fetchAndRenderMarkers() {
            ...
            fetch(url).then(response => response.json()).then(data => {
                ...
                if (currentUserLat && currentUserLng) {
                    ...
                    data.sort((a, b) => {
                        let aHasRating = a.review_count > 0 ? 1 : 0;
                        let bHasRating = b.review_count > 0 ? 1 : 0;
                    ...
                    if (spklu.latitude && spklu.longitude) {
                        ...
                        let popupContent = `
                            <div class="p-2 min-w-[200px] font-sans">
                                ...
                                <a href="/rider/spklu/${spklu.id}" class="block w-full bg-slate-800 hover:bg-slate-700 text-white text-center font-bold text-[10px] py-1.5 px-3 rounded-lg transition-colors mt-2 uppercase tracking-widest">
                                    Fasilitas & Ulasan
                                </a>
                            </div>
                        `;

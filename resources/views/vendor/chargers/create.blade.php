    ...
    <div class="bg-slate-900/40 border border-slate-700/50 rounded-[2rem] p-6 md:p-8 backdrop-blur-xl shadow-xl">
        <form action="{{ route('vendor.chargers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-slate-800/30 border border-slate-700/40 rounded-2xl p-6">
                ...
                <div class="grid gap-6 md:grid-cols-2 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="address" id="address" value="{{ old('address') }}" placeholder="Geser pin di peta untuk mengisi alamat..." readonly required class="w-full px-4 py-3 text-sm border border-slate-700/50 rounded-xl bg-slate-900/80 text-slate-400 placeholder-slate-600 cursor-not-allowed focus:outline-none transition-colors">
                    </div>
                </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3 mt-6">
                ...
                <button type="submit" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-400 text-white px-8 py-3.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:shadow-[0_0_25px_rgba(52,211,153,0.6)] hover:-translate-y-0.5">Simpan Infrastruktur</button>
            </div>
        </form>
    </div>
</div>
            ...
            fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`)
                ...
                .then(data => {
                    if (data && data.display_name) {
                        ...
                        addressInput.value = '';
                        alert('Alamat spesifik tidak ditemukan. Silakan geser pin ke area jalan terdekat.');
                    }
                })
                .catch(error => {
                    console.error('Geocoding Error:', error);
                    addressInput.value = '';
                    alert('Gagal mengambil data alamat. Silakan geser pin ulang.');
                });
        });

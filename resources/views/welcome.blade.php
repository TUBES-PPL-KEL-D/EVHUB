...
<body class="text-slate-800 antialiased min-h-screen flex flex-col relative selection:bg-emerald-500 selection:text-white bg-slate-900 overflow-x-hidden">
        ...
        <div class="flex items-center space-x-4">
            @auth
                @if(auth()->user()->role === 'admin')
                    <a href="{{ route('admin.dashboard') }}" class="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-full text-sm font-bold transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40">Pusat Kendali Admin</a>
                @elseif(auth()->user()->role === 'vendor')
                    <a href="{{ route('vendor.dashboard') }}" class="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-full text-sm font-bold transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40">Dashboard Vendor</a>
                @else
                    <a href="{{ route('rider.map') }}" class="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-full text-sm font-bold transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40">Buka Peta SPKLU</a>
                @endif
            @else
    ...
    <header class="relative min-h-[90vh] flex items-center justify-center px-6 text-center z-10 pt-10 pb-20">
        <div class="max-w-4xl reveal active">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                @auth
                    @if(auth()->user()->role === 'admin')
                        <a href="{{ route('admin.dashboard') }}" class="w-full sm:w-auto bg-white text-slate-900 hover:bg-slate-100 px-8 py-4 rounded-full text-base font-bold transition-all hover:-translate-y-1">Masuk Pusat Kendali</a>
                    @elseif(auth()->user()->role === 'vendor')
                        <a href="{{ route('vendor.dashboard') }}" class="w-full sm:w-auto bg-white text-slate-900 hover:bg-slate-100 px-8 py-4 rounded-full text-base font-bold transition-all hover:-translate-y-1">Buka Dashboard Vendor</a>
                    @else
                        <a href="{{ route('rider.map') }}" class="w-full sm:w-auto bg-white text-slate-900 hover:bg-slate-100 px-8 py-4 rounded-full text-base font-bold transition-all hover:-translate-y-1">Lanjut Cari SPKLU</a>
                    @endif
                @else
                    <a href="{{ route('register') }}" class="w-full sm:w-auto bg-white text-slate-900 hover:bg-slate-100 px-8 py-4 rounded-full text-base font-bold transition-all hover:-translate-y-1">Mulai Perjalanan</a>
                @endauth
    ...
    <section class="py-24 px-6 relative z-10 text-center">
        <div class="max-w-4xl mx-auto bg-gradient-to-br from-emerald-900/40 to-slate-800/40 border border-emerald-500/20 rounded-[3rem] p-10 md:p-16 backdrop-blur-md reveal">
            ...
            @auth
                @if(auth()->user()->role === 'admin')
                    <a href="{{ route('admin.dashboard') }}" class="inline-block bg-emerald-500 hover:bg-emerald-400 text-slate-900 px-10 py-4 rounded-full text-lg font-bold transition-all shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:shadow-[0_0_40px_rgba(16,185,129,0.5)] hover:-translate-y-1">Buka Pusat Kendali</a>
                @elseif(auth()->user()->role === 'vendor')
                    <a href="{{ route('vendor.dashboard') }}" class="inline-block bg-emerald-500 hover:bg-emerald-400 text-slate-900 px-10 py-4 rounded-full text-lg font-bold transition-all shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:shadow-[0_0_40px_rgba(16,185,129,0.5)] hover:-translate-y-1">Buka Dashboard Vendor</a>
                @else
                    <a href="{{ route('rider.map') }}" class="inline-block bg-emerald-500 hover:bg-emerald-400 text-slate-900 px-10 py-4 rounded-full text-lg font-bold transition-all shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:shadow-[0_0_40px_rgba(16,185,129,0.5)] hover:-translate-y-1">Eksplorasi Peta SPKLU</a>
                @endif
            @else
                <a href="{{ route('register') }}" class="inline-block bg-emerald-500 hover:bg-emerald-400 text-slate-900 px-10 py-4 rounded-full text-lg font-bold transition-all shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:shadow-[0_0_40px_rgba(16,185,129,0.5)] hover:-translate-y-1">Buat Akun Gratis</a>
            @endauth
        </div>
    </section>

...
@section('content')
    ...
    <div class="vendor-scope" style="color: #0f172a;">
        ...
        <div class="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
            <div class="border-b border-slate-200 bg-slate-50 px-6 py-6 text-slate-900">
                <h1 class="mt-2 text-3xl font-bold text-slate-900">Status Pendaftaran Vendor</h1>
                <p class="mt-2 text-slate-600">Pantau status pendaftaran perusahaan Anda secara real-time.</p>
            </div>
            ...
            <div class="space-y-6 px-6 py-6">
                ...
                <div class="rounded-2xl bg-slate-50 p-5">
                    <p class="text-sm font-semibold text-slate-700">Status Saat Ini</p>
                    <p class="mt-2 inline-flex rounded-full px-4 py-1.5 text-sm font-semibold {{ $statusClass }}">{{ $vendor->status }}</p>
                </div>

                <div class="rounded-2xl border border-slate-200 bg-white p-5">
                    @if ($isApproved)
                        <p class="text-base font-bold text-emerald-800">Pendaftaran Anda sudah disetujui.</p>
                        <p class="mt-2 text-sm text-slate-700">Vendor dapat melanjutkan ke proses operasional berikutnya di EV-HUB.</p>
                    @elseif ($isRejected)
                        <p class="text-base font-bold text-red-800">Dokumen legalitas Anda ditolak.</p>
                        <p class="mt-2 text-sm text-slate-700">Silakan perbaiki dokumen dan unggah ulang melalui tombol perbaikan.</p>
                    @else
                        <p class="text-base font-bold text-amber-800">Pendaftaran Anda masih dalam proses review.</p>
                        <p class="mt-2 text-sm text-slate-700">Status akan berubah menjadi Approved setelah verifikasi admin selesai.</p>
                    @endif
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3 border-t border-slate-200 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex gap-3">
                    @if ($isApproved)
                        <a href="{{ route('vendor.dashboard') }}" class="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700">Buka Dashboard Vendor</a>
                    @endif
                    @if ($isRejected)

...
@section('content')
    <div class="vendor-scope">
        ...
        <div class="mb-6">
            <h1 class="mt-2 text-3xl font-bold text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.45)]">Upload Dokumen Legalitas Vendor</h1>
            <p class="mt-2 text-slate-200 drop-shadow-[0_1px_3px_rgba(0,0,0,0.45)]">Unggah dokumen legalitas untuk melanjutkan proses pendaftaran vendor.</p>
        </div>
        ...
        <div class="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
            <div class="border-b border-slate-200 bg-slate-50 px-6 py-4">
                ...
                <p class="mt-1 text-sm font-medium text-slate-700">Perusahaan: {{ $vendorProfile->company_name }}</p>
            </div>
            ...
            <form action="{{ route('vendor.documents.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 px-6 py-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        required
                    >
                    <p class="mt-2 text-sm text-slate-700">Contoh: SK, NIB, NPWP, atau surat legalitas lain sesuai kebutuhan PBI.</p>
                </div>

                <div class="flex flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm font-medium text-slate-600">Dokumen ini akan dipakai untuk verifikasi vendor.</p>
